...
def load_sports_extract_table(extract_type: str, extract_data: str, timestamp: str):
    """Load API data to sports extracts table"""
    with BigQueryWarehouse(gcp_credentials=gcp_credentials) as warehouse:
         warehouse.execute(
            """
            INSERT INTO home-dashboard-396803.sports.extracts (timestamp, extract_type, extract_data) 
            VALUES (%(timestamp)s, %(extract_type)s, %(extract_data:string)s);
            """,
            parameters={"timestamp": timestamp, "extract_type": extract_type, "extract_data": extract_data}        
        )
...
@task(retries=2)
def run_sports_merge_extracts():
    """Merge new sports extracts data into main tables"""
    with BigQueryWarehouse(gcp_credentials=gcp_credentials) as warehouse:
        warehouse.execute(
            "CALL sports.usp_merge_extracts();"
        )
    return True